...
KANBAN_GUIDANCE = (
    ...
    "files outside it unless the task explicitly asks. If the workspace is a "
    "git repository and your task changes tracked project files, preserve any "
    "pre-existing unrelated work, stage only your owned files, run the required "
    "validations, `git commit`, then `git push` the resulting commit(s) to the "
    "configured upstream before `kanban_complete` or `review-required` handoff. "
    "If commit or push fails, block with the exact reason instead of leaving "
    "unpushed local-only work. Include commit SHA and push status in handoff "
    "metadata/comments.\n"
